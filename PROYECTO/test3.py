...
if __name__ =='__main__':
    ...
    while cap.isOpened():
        ...
        for res in results:
            indice_filtro1 = np.where((np.isin(res.boxes.cls.cpu().numpy(), [0])) & (res.boxes.conf.cpu().numpy() > 0.3)) [0]
            boxes1 = res.boxes.xyxy.cpu().numpy() [indice_filtro1].astype(int)
            ...
            indice_filtro2 = np.where((np.isin(res.boxes.cls.cpu().numpy(), [1, 2, 3])) & (res.boxes.conf.cpu().numpy() > 0.3)) [0]
            boxes2 = res.boxes.xyxy.cpu().numpy() [indice_filtro2].astype(int)
            ...
            for xmin, ymin, xmax, ymax, track_id in tracks2:
                ...
                if track_id not in cruza_linea_azul:
                    ...
                    if cruza_azul >= 0:
                        cruza_linea_azul[track_id] = True
            ...
            indice_filtro3 = np.where((np.isin(res.boxes.cls.cpu().numpy(), [6, 7])) & (res.boxes.conf.cpu().numpy() > 0.3)) [0]
            boxes3 = res.boxes.xyxy.cpu().numpy() [indice_filtro3].astype(int)
            ...
            for xmin, ymin, xmax, ymax, track_id in tracks3:
                ...
                if track_id not in cruza_linea_verde_1 or track_id not in cruza_linea_verde_2:
                    ...
                    cruza_verde_2 = (GREEN_LINE_2[1][0] - GREEN_LINE_2[0][0]) * (yc - GREEN_LINE_2[0][1]) - (GREEN_LINE_2[1][1] - GREEN_LINE_2[0][1]) * (xc - GREEN_LINE_2[0][0])
                    if cruza_verde_1 >= 0 or cruza_verde_2 >= 0:
